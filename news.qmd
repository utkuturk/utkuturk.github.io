...
link_line <- function(r){
  L <- character()
  if (!is.na(r$h)) L <- c(L, sprintf('<a href="%s">handout</a>',  r$h))
  if (!is.na(r$a)) L <- c(L, sprintf('<a href="%s">abstract</a>', r$a))
  if (!is.na(r$s)) L <- c(L, sprintf('<a href="%s">slides</a>',   r$s))
  if (!is.na(r$p)) L <- c(L, sprintf('<a href="%s">pdf</a>',      r$p))

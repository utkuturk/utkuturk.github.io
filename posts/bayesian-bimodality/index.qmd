...
The trouble is that "I squinted at the histogram and saw two humps" is not a method. Bimodality is exactly the kind of thing eyes invent, especially when you want it. Remember that [famous tweet](https://x.com/MatthewBJane/status/1623526094979874817)? Famous among nerds like me, anyway. When can you actually *see* a correlation? Forget weak ones up to 0.3, even 0.5 or 0.6 are not visible to the naked eye.
...
::: {.callout-tip}
## in a hurry?
Nothing in this post needs any code other than what is written here. If you do not want to read the derivation and just want to run the test on your own data, skip to [the whole thing in one block](#the-whole-thing-in-one-block) at the bottom and copy that. It is four functions and no dependencies beyond base R: `is_unimodal()`, `prior_prob_unimodal()`, `gibbs_bimodal()`, and `bimodality_test()`.
:::
...
Two cases, and this is the whole theorem.
...
First, what does the sampler believe? For each dataset, the data as a histogram with 100 mixture densities drawn from the posterior on top.
...
The trap is the one I cared about. The posterior spaghetti shows the sampler using both components, and yet essentially every posterior draw lands in $\Omega_0$, so the evidence comes out *against* bimodality ($\log_{10} B_{10} = `r round(results[[3]]$log10_bf, 2)`$). A one-versus-two-component comparison gets this case wrong by construction.
